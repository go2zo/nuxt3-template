      ...
      <Transition name="slide-from-left-to-left">
        <div
          ...
          <div class="w-auto h-full overflow-auto d-bg-header">
            <div
              class="flex items-center justify-between w-full px-0.5 sm:px-2 h-header">
              <button
                class="transition-colors duration-200 focus:outline-none"
                aria-label="backButton"
                @click.stop="mobileBack">
                <IconArrowLeft v-if="!mobileMainNav" class="p-3 w-12 h-12" />
...
const { $doc, $menu } = useNuxtApp();

const layout = computed(() => $doc.layout);
const mobileMainNav = ref(!$doc.layout.value.aside);

watch($menu.visible, (value, old) => {
  if (value && !old && $doc.layout.value.aside) {
